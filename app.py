...
def extraer_wkt(geocercas_list, data):
    # Procesa ambos endpoints (object y customlayers)
    items = data.get('data', {}).get('items', data.get('data', {}).get('geometries', []))
    for geo in items:
        ...
        if 'POINT' in wkt:
            try:
                ...
                geocercas_list.append({'id': str(geo.get('id')), 'name': nombre, 'lat': float(lat_str), 'lng': float(lng_str), 'radius': 250})
            except Exception: pass
        elif 'POLYGON' in wkt:
            try:
                coords_str = wkt.split('((')[1].split(',')[0]
                lng_str, lat_str = coords_str.strip().split(' ')
                geocercas_list.append({'id': str(geo.get('id')), 'name': nombre, 'lat': float(lat_str), 'lng': float(lng_str), 'radius': 800})
            except Exception: pass

def cargar_geocercas_api():
    geocercas = []
    
    # 1. NUEVO ENDPOINT (Objects)
    try:
        url_obj = f"{BASE_URL}/object/list.json"
        res_obj = requests.get(url_obj, params={"key": API_KEY, "limit": 500}, timeout=15)
        extraer_wkt(geocercas, res_obj.json())
    except Exception: pass
    
    # 2. RESPALDO API (CustomLayers)
    if not geocercas:
        try:
            url_custom = f"{BASE_URL}/customlayers_geometries/list.json"
            res_custom = requests.get(url_custom, params={"key": API_KEY, "limit": 500}, timeout=15)
            extraer_wkt(geocercas, res_custom.json())
        except Exception: pass

    # 3. RESPALDO LOCAL (Excel)
    if not geocercas:
    ...
    <div class="card">
        <div class="header">
            ...
            <div class="header-text">
                ...
                <p>Módulo de Sincronización Estricta (V7)</p>
            </div>
...
def procesar_reporte_bg(task_id, params):
    try:
        ...
        # Solo pedir al satélite los minutos de interés para ahorrar tiempo
        minutos_a_descargar = []
        for dt in cuadricula_maestra:
            if is_ignition_on(dt) or is_in_route(dt)[0]:
                minutos_a_descargar.append(dt)

        # Inyectar eventos críticos al listado de minutos
        for ev in eventos_ignicion:
            minutos_a_descargar.append(ev['dt'])
        ...
        for idl in parsed_idles:
            ...
            if dur >= min_ralenti * 60:
                ...
                minutos_a_descargar.append(ral_dt)
        ...
        TASKS[task_id]['msg'] = f"Sincronizando {len(minutos_a_descargar)} puntos satelitales (Gobernador V7)..."
        ...
        TASKS[task_id]['msg'] = "Aplicando Gobernador de Velocidad y Anclaje Estricto..."
        ...
        for i, dt in enumerate(cuadricula_maestra):
            ...
            # Si el motor está apagado o en ralentí, la velocidad es matemáticamente 0.
            if not ign_on or is_in_idle(dt):
                current_speed = 0.0

            current_speed = round(current_speed, 1)

            # Actualizamos memoria solo si el GPS trajo un punto nuevo
            if punto:
