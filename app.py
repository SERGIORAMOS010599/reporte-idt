...
# Generador de eventos con detalle temporal
        eventos_rows = []
        for _, row in segments_raw.iterrows():
            h_ini_dt = datetime.strptime(f"{f_in_raw} {str(row['Hora_Inicio']).strip()}:00", "%Y-%m-%d %H:%M:%S")
            h_fin_dt = datetime.strptime(f"{f_in_raw} {str(row['Hora_Fin']).strip()}:00", "%Y-%m-%d %H:%M:%S")
            
            curr_time = h_ini_dt
            speed = float(str(row['Vel_Max']).replace(' km/h', '').strip()) if str(row['Vel_Max']).replace(' km/h', '').strip().isdigit() else 0
            
            # Si hay movimiento, generamos registro minuto a minuto
            if speed > 0:
                while curr_time <= h_fin_dt:
                    # ... [Aquí generamos fila minuto a minuto]
                    eventos_rows.append([unit_name, curr_time.strftime("%Y-%m-%d %H:%M:%S"), row['Origen'], speed, "Motor encendido / En movimiento", "-", "mapa", 0, 0])
                    curr_time += timedelta(minutes=1)
            else:
                # Si está apagado, registramos el inicio y luego cada 10 minutos
                eventos_rows.append([unit_name, curr_time.strftime("%Y-%m-%d %H:%M:%S"), row['Origen'], 0, "Motor apagado", "Detenido en reposo", "mapa", 0, 0])
