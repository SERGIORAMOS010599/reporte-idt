...
                    </div>
                </div>
<!-- Agrega esto en tu form-content, antes del botón de submit -->
<div class="form-group">
    <label>Geocercas para monitoreo de exceso de velocidad:</label>
...
<script>
    // Nueva función para cargar geocercas
    async function cargarGeocercas() {
        const res = await fetch('/api_geocercas');
        const geofences = await res.json();
        const select = $('#geofence_select');
        select.empty();
        geofences.forEach(g => {
            select.append(new Option(g.name, g.geofence_id));
        });
        select.select2({ placeholder: "Selecciona geocercas para validar velocidad", width: '100%' });
    }
    
    // Al seleccionar una geocerca, crear un input de límite de velocidad dinámico
    $('#geofence_select').on('change', function() {
        const container = $('#speed_limits_container');
        const selected = $(this).select2('data');
        container.empty();
        selected.forEach(s => {
            container.append(`<div><label>Límite en ${s.text}:</label><input type="number" class="geo-limit" data-id="${s.id}" value="60"></div>`);
        });
    });
